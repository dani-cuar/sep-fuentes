...
csv_file_path = 'C:/Users/Daniela Cuartas/Documents/UdeA/Proyecto de grado/audio_sampling_rates.csv'
...
if not os.path.exists(csv_file_path):
    ...
    with open(csv_file_path, mode='w', newline='') as file:
        ...
        writer.writerow(['Audio Filename', 'Sampling Rate (Hz)'])
    ...
    for txt_file in os.listdir(dir_txt):
        if txt_file.endswith('.txt'):
            ...
            if audio_file_path:
                ...
                clip_name = f"{txt_file.split('.')[0]}"
                # Escribir en el archivo CSV el nombre del archivo y la frecuencia de muestreo
                with open(csv_file_path, mode='a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([clip_name, audio.frame_rate])

                with open(os.path.join(dir_txt, txt_file), 'r') as f: # lee el archivo txt
                    for line in f.readlines():
                        parts = line.split('\t') # separa el archivo por tabulaciones, columnas
                        if 'Duet' in parts:
                            ...
                            clip.export(clip_path, format="wav", parameters=["-acodec", "pcm_s32le"]) # little endian 32 bits
